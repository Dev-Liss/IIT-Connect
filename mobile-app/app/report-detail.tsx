...
export default function ReportDetailScreen() {
    ...
    const submitResponse = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Update the report with the new response data
                setReport(data.data);
                setResponseText("");
                Alert.alert("Success", "Response sent successfully and saved to database");
            } else {
                    ...
                    {report.responses && report.responses.length > 0 && (
                        <View style={styles.section}>
                            <Text style={styles.sectionLabel}>
                                Response History ({report.responses.length})
                            </Text>
                            {report.responses.map((response, index) => (
                                <View key={response._id || index} style={styles.responseCard}>
                                    <View style={styles.responseHeader}>
                                        <Ionicons name="chatbubble-outline" size={16} color="#457b9d" />
                                        <Text style={styles.responseAdmin}>Admin Response</Text>
